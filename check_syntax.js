...
var DP_REGISTRATION_SHEET = 'DP_Registration_Responses';
  ...
  try {
    ...
    switch (action) {
      ...
      case 'getDirectPlacementCandidates':
        result = { success: true, candidates: getDirectPlacementCandidates(), timestamp: new Date().toISOString() };
        break;
      ...
      case 'getDirectAuditLogs':
        result = getDirectAuditLogs(e.parameter.placementId || '');
        break;
...
function onFormSubmit(e) {
  try {
    ...
    if (rawSheetName === DP_REGISTRATION_SHEET) {
      isDPRegForm = true;
    }
    ...
    // Detect if this is a Direct Placement BGV Form submission
    if (rawSheetName === DP_BGV_SHEET) {
      Logger.log("Detected Direct Placement BGV Form Submission");
      syncDirectBGV();
      return;
    }
...
function getDirectPlacementCandidates() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName(DP_MASTER_SHEET);
  if (!sheet) return [];
...
function getDirectPlacementCandidate(placementId) {
  ...
  for (var i = 0; i < cands.length; i++) {
    if (String(cands[i]['placementId'] || '').trim() === String(placementId).trim()) {
      return { success: true, candidate: cands[i] };
    }
  }
...
function syncDirectPlacement() {
  ...
  var regCols = ['submissionId', 'tokenEmail', 'fullName', 'mobileNumber', 'yearOfPassing', 'currentlyWorking', 'experienceType', 'companyName', 'designation', 'ctc', 'submittedAt', 'syncStatus', 'placementId'];
  var dpRegSheet = ensureSheetWithColumns(ss, DP_REGISTRATION_SHEET, regCols);
  
  var masterCols = ['placementId', 'fullName', 'mobileNumber', 'yearOfPassing', 'currentlyWorking', 'experienceType', 'companyName', 'designation', 'ctc', 'offerLetter', 'relievingLetter', 'pfServiceHistory', 'payslip', 'bgvStatus', 'dateOfBirth', 'fatherName', 'alternateContact', 'address', 'pincode', 'courseName', 'batch', 'documentAmount', 'bgvSubmittedAt', 'candidateStatus', 'trackedStatus', 'trackedAt', 'createdAt', 'updatedAt', 'remarks'];
  var dpMasterSheet = ensureSheetWithColumns(ss, DP_MASTER_SHEET, masterCols);
  
  var regData = dpRegSheet.getDataRange().getValues();
  if (regData.length < 2) return { success: true, synced: 0, skipped: 0 };
  
  var regHeaders = regData[0].map(function(h) { return String(h).trim(); });
  var masterHeaders = getSheetHeaders(dpMasterSheet);
  var masterData = dpMasterSheet.getDataRange().getValues();
  
  var existingData = [];
  for (var m = 1; m < masterData.length; m++) {
    existingData.push(rowToObj(masterHeaders, masterData[m]));
  }
    ...
    if (syncStatus !== 'true' && syncStatus !== 'synced') {
      function getVal(keys) {
        for (var i = 0; i < keys.length; i++) {
          if (rowObj[keys[i]]) return String(rowObj[keys[i]]).trim();
        }
        return '';
      }
      
      var nameObj = getVal(['fullName', 'fullname', 'candidateName', 'name', 'Full Name']);
      var mobileObj = getVal(['mobileNumber', 'mobilenumber', 'mobile', 'phone', 'contactNumber', 'contactnumber']);
      var companyObj = getVal(['companyName', 'companyname', 'company', 'organization']);
      var desigObj = getVal(['designation', 'role', 'title']);
      var yopObj = getVal(['yearOfPassing', 'yop']);
      var ctcObj = getVal(['ctc', 'currentCTC', 'salary']);
      var expTypeObj = getVal(['experienceType', 'experience', 'fresherOrExperienced']);
      var workingObj = getVal(['currentlyWorking', 'working']);
      
      // Deduplication: Mobile -> PlacementID -> Name
      var matchedIdx = -1;
      ...
      if (matchedIdx !== -1) {
        ...
        var masterCount = dpMasterSheet.getLastRow() - 1;
        placementId = 'DP' + ('000' + (masterCount + 1 + synced)).slice(-3);
        isNew = true;
      }
      ...
      setByHeader(dpRegSheet, regHeaders, r + 1, 'placementId', placementId);
      setByHeader(dpRegSheet, regHeaders, r + 1, 'syncStatus', 'synced');
      synced++;
    }
  }
...
function syncDirectBGV() {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var bgvCols = ['submissionId', 'placementId', 'mobileNumber', 'offerLetter', 'relievingLetter', 'pfServiceHistory', 'payslip', 'bgvStatus', 'dateOfBirth', 'fatherName', 'alternateContact', 'address', 'pincode', 'courseName', 'batch', 'documentAmount', 'bgvSubmittedAt', 'syncStatus'];
    var bgvSheet = ensureSheetWithColumns(ss, DP_BGV_SHEET, bgvCols);
    
    var masterCols = ['placementId', 'fullName', 'mobileNumber', 'yearOfPassing', 'currentlyWorking', 'experienceType', 'companyName', 'designation', 'ctc', 'offerLetter', 'relievingLetter', 'pfServiceHistory', 'payslip', 'bgvStatus', 'dateOfBirth', 'fatherName', 'alternateContact', 'address', 'pincode', 'courseName', 'batch', 'documentAmount', 'bgvSubmittedAt', 'candidateStatus', 'trackedStatus', 'trackedAt', 'createdAt', 'updatedAt', 'remarks'];
    var masterSheet = ensureSheetWithColumns(ss, DP_MASTER_SHEET, masterCols);
    
    var bgvData = bgvSheet.getDataRange().getValues();
    if (bgvData.length < 2) return { success: true, synced: 0, skipped: 0 };
    
    var masterData = masterSheet.getDataRange().getValues();
    var masterHeaders = masterData[0].map(function(h) { return String(h).trim(); });
    
    // Cache master candidates by placementId and mobileNumber to avoid nested loops
    var candidateMap = {};
    var mobileMap = {};
    for (var i = 1; i < masterData.length; i++) {
      var pId = String(masterData[i][findHeaderCol(masterHeaders, 'placementId')] || '').trim();
      var mob = String(masterData[i][findHeaderCol(masterHeaders, 'mobileNumber')] || '').trim();
      if (pId) candidateMap[pId] = i + 1; // 1-indexed row number
      if (mob) mobileMap[mob] = i + 1;
    }
    ...
    for (var r = 1; r < bgvData.length; r++) {
      ...
      if (syncStatus !== 'true' && syncStatus !== 'synced') {
        var pId = String(rowObj['placementId'] || '').trim();
        var mob = String(rowObj['mobileNumber'] || '').trim();
        
        var targetRow = candidateMap[pId] || mobileMap[mob];
        
        if (targetRow) {
          var now = new Date().toISOString();
          
          setByHeader(masterSheet, masterHeaders, targetRow, 'bgvStatus', rowObj['bgvStatus'] || 'submitted');
          setByHeader(masterSheet, masterHeaders, targetRow, 'dateOfBirth', rowObj['dateOfBirth'] || '');
          setByHeader(masterSheet, masterHeaders, targetRow, 'fatherName', rowObj['fatherName'] || '');
          setByHeader(masterSheet, masterHeaders, targetRow, 'alternateContact', rowObj['alternateContact'] || '');
          setByHeader(masterSheet, masterHeaders, targetRow, 'address', rowObj['address'] || '');
          setByHeader(masterSheet, masterHeaders, targetRow, 'pincode', rowObj['pincode'] || '');
          setByHeader(masterSheet, masterHeaders, targetRow, 'courseName', rowObj['courseName'] || '');
          setByHeader(masterSheet, masterHeaders, targetRow, 'batch', rowObj['batch'] || '');
          setByHeader(masterSheet, masterHeaders, targetRow, 'documentAmount', rowObj['documentAmount'] || '');
          setByHeader(masterSheet, masterHeaders, targetRow, 'bgvSubmittedAt', rowObj['bgvSubmittedAt'] || now);
          setByHeader(masterSheet, masterHeaders, targetRow, 'updatedAt', now);
    ...
    Logger.log("syncDirectBGV Error: " + err.message);
...
function updateDirectCandidate(params) {
  try {
    ...
    logDirectAudit(ss, placementId, existingName, 'Candidate Profile', 'Update', '', '', params.userStamp || 'System');
    SpreadsheetApp.flush();
...
function upsertDirectFinancialLedger(ss, placementId, candidateName) {
  if (!placementId) return;
  var pSheet = ss.getSheetByName(DP_PAYMENT_SHEET);
  var lCols = ['placementId', 'candidateName', 'registrationFee', 'documentFee', 'otherCharges', 'discount', 'totalFee', 'paidAmount', 'pendingAmount', 'paymentStatus', 'updatedAt'];
    ...
    if (pData.length > 1) {
      ...
      for (var i=1; i<pData.length; i++) {
        if (String(pData[i][pIdIdx]).trim() === String(placementId).trim()) {
          totalPaid += parseFloat(pData[i][amtIdx]) || 0;
        }
      }
    }
  }
  ...
  var lHeaders = lData[0].map(function(h){ return String(h).trim(); });
  
  var targetRow = -1;
  var regFee = 0, docFee = 0, other = 0, discount = 0;
  var targetRow = -1;
  var regFee = 0, docFee = 0, other = 0, discount = 0;
  ...
  if (lData.length > 1) {
    ...
    for (var j=1; j<lData.length; j++) {
      if (String(lData[j][lIdIdx]).trim() === String(placementId).trim()) {
        ...
        other = parseFloat(lData[j][findHeaderCol(lHeaders, 'otherCharges')]) || 0;
        discount = parseFloat(lData[j][findHeaderCol(lHeaders, 'discount')]) || 0;
        break;
      }
    }
  }
  
  var pending = calculatePendingAmount(regFee, docFee, other, discount, totalPaid);
  var totalFee = (regFee + docFee + other) - discount;
  var status = updatePaymentStatus(pending, totalPaid);
  var now = new Date().toISOString();
  
  if (targetRow !== -1) {
    setByHeader(lSheet, lHeaders, targetRow, 'totalFee', totalFee);
    setByHeader(lSheet, lHeaders, targetRow, 'paidAmount', totalPaid);
...
function addDirectPayment(params) {
  try {
    ...
    sheet.appendRow(row);
    logDirectAudit(ss, placementId, candidateName, 'Finance', 'Add Payment', '', '₹' + amount, params.userStamp || 'System');
...
function updateDirectPayment(params) {
  try {
    ...
    logDirectAudit(ss, placementId, candidateName, 'Finance', 'Update Payment', '', 'Updated ' + paymentId, params.userStamp || 'System');
...
function deleteDirectPayment(params) {
  try {
    ...
    logDirectAudit(ss, placementId, candidateName, 'Finance', 'Delete Payment', paymentId, 'Deleted', params.userStamp || 'System');
