      ...
      <nav className="fixed top-0 left-0 right-0 bg-cc-base-deep border-b border-cc-gridline z-[100] shadow-[0_12px_40px_rgba(0,0,0,0.32)]">
        <div className="max-w-[1200px] mx-auto h-14 flex items-center justify-between px-4 lg:px-6">
          <div className="flex items-center gap-4">
            <Link to={location.pathname.startsWith('/dp') ? "/dp" : "/"} className="font-mono text-xs font-medium tracking-[0.12em] uppercase text-cc-text-high hover:text-cc-warm-text transition-colors">
              {location.pathname.startsWith('/dp') ? 'DIRECT PLACEMENT CRM' : 'COMMAND CENTER'}
            </Link>
            ...
            <div className="hidden sm:flex items-center gap-4">
              ...
              <Link
                to="/dp"
                className={`nav-label transition-colors ${isActive('/dp') ? 'text-cc-text-high' : 'text-cc-text-mid hover:text-cc-warm-text'}`}
              >
                Direct Placement
              </Link>
              <Link
                to="/backup"
                className={`nav-label transition-colors ${location.pathname.startsWith('/backup') ? 'text-cc-text-high' : 'text-cc-text-mid hover:text-cc-warm-text'}`}
              >
                Backup
              </Link>
            </div>
        ...
        {menuOpen && (
          <div className="sm:hidden bg-cc-base-deep border-t border-cc-gridline px-4 py-3 space-y-2">
            ...
            <Link
              to="/dp"
              onClick={() => setMenuOpen(false)}
              className={`block nav-label py-2 ${isActive('/dp') ? 'text-cc-text-high' : 'text-cc-text-mid'}`}
            >
              Direct Placement
            </Link>
            <Link
              to="/backup"
              onClick={() => setMenuOpen(false)}
              className={`block nav-label py-2 ${location.pathname.startsWith('/backup') ? 'text-cc-text-high' : 'text-cc-text-mid'}`}
            >
              Backup
            </Link>
          </div>
