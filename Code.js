...
function getDirectPlacementCandidates(params) {
  ...
  var headerMap = getDirectPlacementHeaderMap(sheet);
  var candidates = [];
  ...
  for (var i = masterHeaderRowIdx + 1; i < data.length; i++) {
    ...
    var obj = directPlacementRowToObj(row, headerMap);
...
function upsertDirectFinancialLedger(ss, placementId, candidateName) {
    ...
    if (pData.length > 1) {
      ...
      for (var i=1; i<pData.length; i++) {
        if (String(pData[i][pIdIdx]).trim() === String(placementId).trim()) {
          ...
          totalPaid += amt;
          var pt = String(pData[i][typeIdx]).toLowerCase().replace(/fee/g, '').replace(/[^a-z]/g, '');
  ...
  var targetRow = -1;
  var regFee = 0, docFee = 0, courseFee = 0, placementFee = 0, other = 0, discount = 0;
  ...
  var pending = Math.max(0, (regFee + docFee + courseFee + placementFee + other) - discount - totalPaid);
  var totalFee = (regFee + docFee + courseFee + placementFee + other) - discount;
...
function uploadDPDocument(params) {
  try {
    ...
    if (data.length > 1 && String(data[1][0] || '').trim().toLowerCase() === 'placementid') {
      headerRowIdx = 1;
      headers = data[1].map(function(h) { return String(h).trim(); });
    }
...
function syncDirectBGV(e) {
  try {
    ...
    var masterHeaderRowIdx = 0;
    if (mData.length > 1 && String(mData[1][0] || '').trim().toLowerCase() === 'placementid') {
      masterHeaderRowIdx = 1;
    }
    
    var headerRow = mData[masterHeaderRowIdx].map(function(h) { return String(h).trim(); });
    var headerMap = getDirectPlacementHeaderMap(mSheet);
...
// ============================================================
function getDirectPlacementHeaderMap(sheet) {
  var data = sheet.getDataRange().getValues();
  if (data.length < 2) return {};
  ...
  for (var i = 0; i < headerRow.length; i++) {
    ...
    // Only store the first occurrence of every header
...
function directPlacementRowToObj(row, headerMap) {
  var obj = {};
