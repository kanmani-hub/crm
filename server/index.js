...
app.post('/api/append', (req, res) => {
  ...
  try {
    ...
    if (sheetName === 'BGV_Responses') {
      ...
      const nowString = new Date().toISOString();
      const email = (values.email || rowObject.email || '').toLowerCase();
      const bvgCandId = values.candidateId || rowObject.candidateId || rowObject.id;
      ...
      if (candidate) {
        ...
        if (values.address || rowObject.address) candidate.address = values.address || rowObject.address;
        
        // Map extra frontend BGV fields to candidate if they exist
        if (values.aadhar) candidate.aadhar = values.aadhar;
        if (values.emergencyContact) candidate.alternateContactNumber = values.emergencyContact;
        ...
        docList.forEach(doc => {
          if (String(doc.candidateId) === String(candidateId) && docKeys.includes(doc.documentKey)) {
            // Check matching value from BGV response documents object
            const formVal = (values.documents && values.documents[doc.documentKey]) || rowObject[doc.documentKey];
        ...
        if (Array.isArray(companies)) {
          companies.forEach((comp, index) => {
            ...
            const bvgCompRow = {
              responseId: values.responseId || rowObject.responseId || `bgv_${Date.now()}`,
              candidateId: candidateId,
              sortOrder: index + 1,
        ...
        console.log(`Candidate not found for BGV submission: ID ${bvgCandId}, Email ${email}`);
      }
    }
