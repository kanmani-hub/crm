...
import DPDashboard from './pages/directPlacement/DPDashboard';
import DPCandidateProfile from './pages/directPlacement/DPCandidateProfile';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    import('./store/useDPStore').then(({ useDPStore }) => {
      useDPStore.getState().fetchInitialData();
    });
    
    // Automatically poll for new data (e.g. from background Google Sheets sync)
    const interval = setInterval(() => {
      useStore.getState().fetchInitialData(true, true);
      import('./store/useDPStore').then(({ useDPStore }) => {
        useDPStore.getState().fetchInitialData(true, true);
      });
  ...
  return (
    <AnimatePresence mode="wait">
      <Routes location={location} key={location.pathname}>
        ...
        <Route path="/dp" element={<ProtectedRoute><PageTransition><DPDashboard /></PageTransition></ProtectedRoute>} />
        <Route path="/dp/candidate/:id" element={<ProtectedRoute><PageTransition><DPCandidateProfile /></PageTransition></ProtectedRoute>} />
        ...
        <Route path="/backup" element={<ProtectedRoute><PageTransition><BackupCenter /></PageTransition></ProtectedRoute>} />
