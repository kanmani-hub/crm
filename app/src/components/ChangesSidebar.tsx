...
export default function ChangesSidebar({ mode = 'training' }: { mode?: 'training' | 'dp' }) {
  const store = useStore();
  const dpStore = useDPStore();
  const activeStore = mode === 'dp' ? dpStore : store;

  const { sidebarOpen, setSidebarOpen, activeProfileId, auditLogs } = activeStore;
  const candidate = mode === 'dp' 
    ? (activeProfileId ? dpStore.candidates.find(c => c.placementId === activeProfileId) : null)
    : (activeProfileId ? store.getCandidateById(activeProfileId) : null);

  const filteredLogs = auditLogs.filter((l) => {
    if (mode === 'dp') {
      return String((l as any).placementId || '').trim().toLowerCase() === String(activeProfileId || '').trim().toLowerCase();
    }
    return (l as any).candidateId === activeProfileId;
  });
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredLogs.length === 0 ? (
                ...
                filteredLogs.map((log) => {
                  const cfg = typeConfig[((log as any).logType as LogType) || 'structural'] || { dot: 'bg-cc-blue', label: (log as any).module || 'STRUCTURAL' };
                  const isDP = mode === 'dp';
                  
                  return (
                    <div key={(log as any).id || (log as any).auditId} className="p-4 border-b border-[rgba(42,48,56,0.5)]">
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${cfg.dot}`} />
                        <span className="font-mono text-[9px] font-medium uppercase tracking-[0.06em] text-cc-text-mid">
                          {isDP ? ((log as any).module || cfg.label) : cfg.label}
                        </span>
                      </div>
                      
                      {isDP && (log as any).action && (
                        <p className="font-mono text-[10px] font-semibold tracking-wider text-white mt-1">
                          {(log as any).action.replace(/_/g, ' ')}
                        </p>
                      )}
                      
                      <p className="font-sans text-[13px] text-cc-text-high mt-1.5">{(log as any).description}</p>
                      
                      {isDP ? (
                        <div className="mt-2 flex gap-4 text-xs">
                           <div className="flex flex-col">
                             <span className="text-cc-text-low font-mono">Old:</span>
                             <span className="text-cc-warm-text font-medium">{(log as any).oldValue || '--'}</span>
                           </div>
                           <div className="flex flex-col">
                             <span className="text-cc-text-low font-mono">New:</span>
                             <span className="text-cc-green font-medium">{(log as any).newValue || '--'}</span>
                           </div>
                        </div>
                      ) : (
                        (log as any).reason && (
                          <p className="font-sans text-xs text-cc-warm-text italic mt-1">{(log as any).reason}</p>
                        )
                      )}
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-full bg-cc-base-elevated flex items-center justify-center font-mono text-[8px] text-cc-text-mid">
                            {(isDP ? (log as any).user || (log as any).performedBy || 'SY' : (log as any).userStamp).slice(0, 2)}
                          </span>
                          <span className="micro-text text-cc-text-mid">{isDP ? (log as any).user || (log as any).performedBy || 'System' : (log as any).userStamp}</span>
                        </div>
                        <span className="micro-text text-cc-text-low">{formatDate((log as any).timestamp)}</span>
                      </div>
                    </div>
                  );
                })
