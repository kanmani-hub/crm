...
import { Mail, FileText, Shield, Send, Pencil, Check, X, Plus, Database, RefreshCw, Users, UserPlus, IndianRupee, Activity, CheckCircle } from 'lucide-react';
...
const toggles: { type: PayloadType; label: string; icon: typeof FileText; color: string }[] = [
  ...
  { type: 'direct-placement-form', label: 'DP REG FORM', icon: FileText, color: '#A04B32' },
  { type: 'bgv-form', label: 'BGV FORM', icon: Shield, color: '#5B8FBF' },
  { type: 'direct-bgv-form', label: 'DP BGV FORM', icon: Shield, color: '#4A7A9F' },
  ...
  const handleSend = async () => {
    ...
    try {
      if (isContactMail) {
        ...
        setEmail('');
      } else if (selectedToggle === 'direct-placement-form') {
        const formLink = settings.googleSheetLinks.dpRegistrationForm || 'https://forms.gle/LfD5SxxYXDqFGMwn6';
        ...
        showToast('DP Registration link dispatched invisibly in the background.');
        
        addTrackedCandidate({
          candidateId: `tc_${Date.now()}`,
          status: 'form-pending',
          payloadType: 'direct-placement-form',
          email: email.trim(),
          timestamp: new Date().toISOString(),
        });
        setEmail('');
      } else if (selectedToggle === 'direct-bgv-form') {
        const formLink = settings.googleSheetLinks.dpBgvForm || 'https://forms.gle/LfD5SxxYXDqFGMwn6'; 
        ...
        showToast('DP BGV link dispatched invisibly in the background.');
        
        addTrackedCandidate({
          candidateId: `tc_${Date.now()}`,
          status: 'bgv-submitted',
          payloadType: 'direct-bgv-form',
          email: email.trim(),
          timestamp: new Date().toISOString(),
        });
        setEmail('');
